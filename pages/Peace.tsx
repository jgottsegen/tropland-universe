...
export default function Peace({reviewMode=false,assetBase='/images/peace/'}:{reviewMode?:boolean;assetBase?:string}) {
 ...
 return <div className="peace-page">
  ...
  <main id="peace-main">
   <section className="peace-hero" aria-labelledby="peace-title">
    <div className="peace-gallery">
     ...
     <div className="peace-gallery-bottom"><div className="peace-thumbnails">{gallery.map((photo,index)=><button type="button" key={photo.url} aria-label={`Show ${color} mockup ${index+1}`} aria-pressed={view==='shirt'&&imageIndex===index} onClick={()=>{setImageIndex(index);setView('shirt');}}><img src={photo.url} alt="" width="48" height="64" loading="lazy"/></button>)}<button type="button" aria-label="Show artwork detail" aria-pressed={view==='detail'} onClick={()=>setView('detail')}><img src={`${assetBase}lion-signed-brown.webp`} alt="" width="48" height="60"/></button></div><p>Product mockup.<br/>Screen colors may vary.</p></div>
    </div>
    <div className="peace-product" id="peace-buy">
     ...
     <div className="peace-buy-row"><label className="peace-quantity">Qty<select aria-label="Quantity" value={qty} onChange={e=>setQty(Number(e.target.value))}>{Array.from({length:10},(_,i)=><option key={i+1} value={i+1}>{i+1}</option>)}</select></label><button type="button" className="peace-button" onClick={buy} disabled={busy||liveState==='loading'||checkoutPending}>{busy?'Opening checkout…':liveState==='loading'?'Checking availability…':checkoutPending?'Checkout opening soon':`Buy the Peace Lion${size?` · ${money(price*qty)}`:''}`}<span aria-hidden="true">↗</span></button></div>
  ...
  <footer className="peace-footer"><img src={`${assetBase}tropland-horizontal-white.svg`} alt="Tropland Universe" width="270" height="46"/><span>The Digital Animal Kingdom</span><a href="https://www.troplanduniverse.com/">Explore the universe ↗</a></footer>
  <div className="peace-mobile-buy"><div><strong>{money(price*qty)}</strong><span>{color}{size?` / ${size}`:' / Choose a size'}</span></div><button type="button" className="peace-button" disabled={busy||liveState==='loading'||checkoutPending} onClick={buy}>{busy?'Opening…':checkoutPending?'Opening soon':'Get yours'} <span aria-hidden="true">↗</span></button></div>
