...
export default function Peace({reviewMode=false,assetBase='/images/peace/'}:{reviewMode?:boolean;assetBase?:string}) {
 ...
 return <div className="peace-page">
  ...
  <main id="peace-main">
   <section className="peace-mobile-intro" aria-label="Meet the Peace Lion Tee">
    <a href="#peace-buy" className="peace-mobile-campaign" aria-label="Find your Peace Lion Tee color"><img src="/images/peace/peace-lion-campaign-v1.jpg" alt="A little wild. A lot of peace. The Peace Lion Tee, featuring the original golden lion artwork." width="1734" height="907" fetchPriority="high"/></a>
    <div className="peace-mobile-intro-bar"><p>Wear a little peace.</p><a href="#peace-buy">Find your color <span aria-hidden="true">↓</span></a></div>
   </section>
  ...
  <div className="peace-mobile-buy"><div><strong>{money(price*qty)}</strong><span>{color}{size?` / ${size}`:' / Choose a size'}</span></div><button type="button" className="peace-button" disabled={busy||liveState==='loading'||checkoutPending} onClick={buy}>{busy?'Opening…':checkoutPending?'Opening soon':'Get yours'} <span aria-hidden="true">↗︎</span></button></div>
