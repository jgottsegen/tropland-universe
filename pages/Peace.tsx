  ...
  <main id="peace-main">
   ...
   <section className="peace-hero" aria-labelledby="peace-title">
    <div className="peace-gallery">
     ...
     <div className="peace-gallery-bottom"><details className="peace-gallery-details"><summary>View all photos</summary><div className="peace-thumbnails">{gallery.map((photo,index)=><button type="button" key={photo.url} aria-label={`Show ${color} mockup ${index+1}`} aria-pressed={view==='shirt'&&imageIndex===index} onClick={()=>{setImageIndex(index);setView('shirt');}}><img src={photo.url} alt="" width="48" height="64" loading="lazy"/></button>)}<button type="button" aria-label="Show artwork detail" aria-pressed={view==='detail'} onClick={()=>setView('detail')}><img src={`${assetBase}lion-signed-brown-v2-20260917.webp`} alt="" width="48" height="60"/></button></div></details><p>Screen colors may vary.</p></div>
    </div>
