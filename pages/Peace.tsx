...
export default function Peace({reviewMode=false,assetBase='/images/peace/'}:{reviewMode?:boolean;assetBase?:string}) {
 ...
 return <div className="peace-page">
  ...
  <main id="peace-main">
   <div className="peace-mobile-heading"><p>STRONG ENOUGH<br/><em>TO CHOOSE PEACE</em></p></div>
   <section className="peace-hero" aria-labelledby="peace-title">
    <div className="peace-gallery">
     <div className="peace-image-wrap"><span className="peace-image-label">{view==='shirt'?`${color} / View ${imageIndex+1} of ${gallery.length}`:'The signed artwork'}</span>
      <img className={view==='shirt'?'peace-shirt-image':'peace-art-image'} src={view==='shirt'?image:`${assetBase}lion-signed-brown-v2-20260917.webp`} alt={view==='shirt'?`Peace Lion Tee in ${color}, product mockup ${imageIndex+1} of ${gallery.length}`:'Close view of the original lion illustration and curved artist signature'} width={view==='shirt'?720:1400} height={view==='shirt'?960:1763} fetchPriority="high"/>
     </div>
     <div className="peace-gallery-bottom"><div className="peace-thumbnails" role="group" aria-label={`${color} product photos`}>{gallery.map((photo,index)=><button type="button" key={photo.url} aria-label={`Show ${color} mockup ${index+1}`} aria-pressed={view==='shirt'&&imageIndex===index} onClick={()=>{setImageIndex(index);setView('shirt');}}><img src={photo.url} alt="" width="48" height="64" loading="lazy"/></button>)}<button type="button" aria-label="Show artwork detail" aria-pressed={view==='detail'} onClick={()=>setView('detail')}><img src={`${assetBase}lion-signed-brown-v2-20260917.webp`} alt="" width="48" height="60"/></button></div><p>Screen colors may vary.</p></div>
    </div>
    <div className="peace-product" id="peace-buy">
     <p className="peace-eyebrow">Tropland Universe / Wearable art</p><h1 id="peace-title">Peace Lion Tee</h1><p className="peace-headline">STRONG ENOUGH<br/><em>TO CHOOSE PEACE</em></p>
     <div className="peace-price" aria-live="polite">{!size&&<span>From </span>}{money(price)} <small>USD</small></div>
     <p className="peace-description">The strength of a lion. The choice of peace. Original golden artwork and a subtle artist’s signature on a combed cotton tee.</p>
   ...
   <section className="peace-story" aria-labelledby="peace-story-title"><div className="peace-story-art"><img src={`${assetBase}lion-signed-gold-v2-20260917.webp`} alt="The golden lion peace illustration, with a small signature below its lower-right edge" width="1400" height="1763" loading="lazy"/></div><div><p className="peace-eyebrow">Art with a point of view</p><h2 id="peace-story-title">Wear your strength.<br/><em>Choose your peace.</em></h2><p>A lion. A peace sign. A statement of quiet confidence. Wear your strength with the courage to choose peace.</p><p>Fine linework, warm gold and an artist’s signature make this a piece to look closer at. The AS Colour tee keeps the rest simple: combed cotton, a classic crew neck and a regular fit.</p><a href="#peace-buy">Choose your color <span aria-hidden="true">↑</span></a></div></section>
   <section className="peace-color-story" aria-labelledby="peace-color-title"><div className="peace-section-heading"><div><p className="peace-eyebrow">Make it yours</p><h2 id="peace-color-title">Same peace. Your color.</h2></div><span>{catalog.colors.length} ways to wear it</span></div><div className="peace-color-grid">{['Black','White','Army'].filter(name=>catalog.colors.some(c=>c.name===name)).map(name=><button type="button" key={name} onClick={()=>{chooseColor(name);document.getElementById('peace-buy')?.scrollIntoView({behavior:'smooth'});}}><img src={reviewMode?`${assetBase}mockup-${name.toLowerCase()}-v2-20260917.jpg`:catalog.colors.find(c=>c.name===name)?.image} alt={`Peace Lion Tee in ${name}`} loading="lazy" width="720" height="960"/><span>{name}<span aria-hidden="true">↗︎</span></span></button>)}</div></section>
